...
test("GET /admintasks/:id 401", async () => {
  const { status } = await request(app()).get(`${apiRoot}/${admintask.id}`);
  expect(status).toBe(401);
});
...
test("PUT /admintasks/:id 401", async () => {
  const { status } = await request(app()).put(`${apiRoot}/${admintask.id}`);
  expect(status).toBe(401);
});
...
test("DELETE /admintasks/:id 401", async () => {
  const { status } = await request(app()).delete(`${apiRoot}/${admintask.id}`);
  expect(status).toBe(401);
});
