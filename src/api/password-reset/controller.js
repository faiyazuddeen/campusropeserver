...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your campusropeserver account.<br>
        ...
        &mdash; campusropeserver Team
      `
      return sendMail({ toEmail: email, subject: 'campusropeserver - Password Reset', content })
    })
